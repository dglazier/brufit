{
  ...
  Flat.SetUp().FactoryPDF("RooComponentsPDF::Flat(1,{CosThA1,PhiA1,CosThB1,PhiB1,PolPhi,Pol},=Val)");
  ...
  ComponentsPdfParser  parser=HS::FIT::PolWignerDFunctionProductMoments("Moments","ThetaA1","CosThA1","PhiA1","ThetaB1","CosThB1","PhiB1","PolPhi","Pol",2,0,2,0,2,2);
  ...
  Generator.SetUp().LoadConstant("H0_0_0_0_0[1]");
  string sum;//="K_0*K_0*h_0_0_0_0[1]";
  sum +=     "SUM(L[0|2],l[0|2],M[0|2<L+1],m[0|2<L+1<l+1]){K_l*K_L*H0_l_m_L_M[0,-1,1]*ReDab_l_m_L_M}";
  
  sum +=       "+ SUM(L[0|2],l[0|2],M[0|2<L+1],m[0|2<L+1<l+1]){K_l*K_L*H1_l_m_L_M[0,-1,1]*ReDab_l_m_L_M*COS2PHI}";
  sum+=        "+ SUM(L[1|2],l[0|2],M[1|2<L+1],m[0|2<L+1<l+1]){K_l*K_L*H2_l_m_L_M[0,-1,1]*ImDab_l_m_L_M*SIN2PHI}";
  ...
  Generator.SetUp().WS().var("H0_2_2_2_2")->setVal(0.6);
  Generator.SetUp().WS().var("H1_1_0_1_0")->setVal(0.2);
  Generator.SetUp().WS().var("H2_2_2_2_2")->setVal(-0.4);
  ...
  auto mcmcCov=new RooMcmcSeqThenCov(20000,10000,500,10000,5000,1);
  mcmcCov->SetDesiredAcceptance(0.15,0.3,0.23);
  mcmcCov->SetUncorrelateYields(1);
  Fitter->SetMinimiser(mcmcCov);
