...
namespace bru {
    ...
    void BruEventsHistPeakPDF::FillBase1DHist(TH1D& his1) {
        ...
        // select symmetric region around peak to use adaptive binning
        ...
        std::cout << maxBin << " " << his1.GetBinCenter(maxBin) << " " << xmin << " " << xmax << std::endl;
	...
	peakhist.SetDirectory(nullptr);
        ...
        // these are dependent on the bin content
        ...
        // loop over old bins and create new bins array
        for (auto i = 1; i <= peakhist.GetNbinsX(); i++) {
            // This line scales the bin widths dependent on their content
        ...
        for (auto& frac : fracs) {
            if (frac == 2) frac = minFrac; // correct in case of zero bin contents
            sumFracs += frac;
        }
        for (auto& frac : fracs) {
            frac /= sumFracs;
        ...
        // now need to fill out rest with initial binning scheme.
        double edge = bins[0];
        Int_t gradual = 3;
        while (edge > xmin0) {
            // gradually increase bin width over 3 bins
