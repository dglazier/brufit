...
namespace FIT{
      ...
      fNIntSamples(nsamp)
    {
      ...
      std::cout << "RooHSEventsHistPDF::RooHSEventsHistPDF using fudge parameters:" << std::endl;
      ...
      else std::cout << "RooHSEventsHistPDF::RooHSEventsHistPDF Warning no scale minimum set take = 1" << std::endl;
      ...
      std::cout << "RooHSEventsHistPDF::RooHSEventsHistPDF using binning (" << NbinX << ", " << rMin << ", " << rMax << ") for x-axis variable '" << _x.GetName() << "' of PDF '" << name << "'" << std::endl;
      ...
      alpha("alpha", this, other.alpha)
    {
      ...
      if (fRHist) fRHist->SetDirectory(nullptr);
      
      MakeSets(); // Internal InitSets() resets fLast

      // FIX: Restore cache variables after MakeSets() clears them
      fLast = other.fLast;
      fLastLength = other.fLastLength;
    }

    RooHSEventsHistPDF::~RooHSEventsHistPDF() {
      if (fHist) delete fHist;
      // FIX: Explicitly delete fRHist. Import(*fRHist) does not take ownership.
    ...
    Bool_t RooHSEventsHistPDF::SetEvTree(TTree* tree, TString cut, TTree* MCGenTree) {
      Bool_t OK = RooHSEventsPDF::SetEvTree(tree, cut, MCGenTree);
      if (!fHist) CreateHistPdf();
      return (OK && fHist) ? kTRUE : kFALSE;
    }
    ...
    void RooHSEventsHistPDF::CreateHistPdf() {
      ...
      for (Int_t ia = 2; ia <= fRHist->GetNbinsY(); ia++) {
      ...
      for (Int_t ia = 1; ia <= fRHist->GetNbinsY(); ia++) {
        Double_t max_cont = 0;
        for (Int_t jtemp = 1; jtemp <= fRHist->GetNbinsX(); jtemp++) {
    ...
    Double_t RooHSEventsHistPDF::analyticalIntegral(Int_t code, const char* rangeName) const {
      if (code == 1) {
        ...
        // FIX: Hoist the string parsing out of the 1000-sample integration loop!
    ...
    void RooHSEventsHistPDF::CheckForNegativeBins(TH1D& his1) {
      for (Int_t ix = 0; ix < his1.GetEntries(); ix++) {
        if (his1.GetBinContent(ix) < 0) {
          std::cout << " RooHSEventsHistPDF::CreateHistPdf() weights have resulted in some -ve bins. Will try averaging..." << std::endl;
          Double_t b0, b1, b2;
    ...
    void RooHSEventsHistPDF::generateEvent(Int_t code) {
      if (fUseHistGenerator) {
        ...
        // FIX: Hoist default range string parsing out of the while loop
    ...
    void RooHSEventsHistPDF::initGenerator(Int_t code) {
      ...
      fx_off->setVal(arg);
      falpha->setVal(Double_t(alpha));
      auto gbin = fRHist->FindFixBin(arg, Double_t(alpha));
