/**
 * @file ToyManager.cpp
 * @brief Implementation of the ToyManager class.
 */
...
namespace FIT {
    ...
    ToyManager::ToyManager(const ToyManager& other) : FitManager(other) {
        fToyFileNames = other.fToyFileNames;
        fResultOutDir = other.fResultOutDir;
        fResultFileName = other.fResultFileName;
        fIDval = other.fIDval;
        fNEvents = other.fNEvents;
        fNToys = other.fNToys;
        fToyi = other.fToyi;
        // fGenData is deliberately NOT copied (it is transient)
    }

    ToyManager& ToyManager::operator=(const ToyManager& other) {
        if (this != &other) {
    ...
    void ToyManager::Generate() {
        std::cout << "ToyManager::Generate()" << std::endl;
        ...
        while (fToyi < fNToys) { // Run sequentially
            std::cout << "ToyManager::Generate() " << fToyi << " of " << fNToys << std::endl;
            
            // Use explicit number of events if set, otherwise use expected model yields
            Long64_t NtoGen = fNEvents == -1 ? fCurrSetup->SumOfYields() : fNEvents;
            Long64_t nexp = RooRandom::randomGenerator()->Poisson(NtoGen);
	
            model->Print();
            
            // Generate data and securely pass ownership to unique_ptr
            fGenData.reset(model->generate(fitvars, nexp));
    ...
    void ToyManager::SaveResults() {
        ...
        // FIXED: Safely wrap the TFile pointer from TFile::Open
        std::unique_ptr<TFile> outfile(TFile::Open(fileName.Data(), "recreate"));
        
        // Convert RooDataSet to TTree safely via unique_ptr
        std::unique_ptr<TTree> tree(RooStats::GetAsTTree("ToyData", "ToyData", *fGenData));

        auto numEntries = fGenData->numEntries();
        auto cats = fCurrSetup->Cats();

        // Add categories to tree (not done by GetAsTTree)
        std::vector<TBranch*> branches(cats.getSize());
        std::vector<Int_t> branchVal(cats.getSize());
        Int_t ib = 0;
        ...
        // Loop over dataset to fill category/ID branches
        ...
        for (auto* pdf : PDFs) {
            if (auto* evPdf = dynamic_cast<RooHSEventsPDF*>(pdf)) {
                ...
                if (!entryFile.IsOpen()) continue; 
                
                auto* entryList = dynamic_cast<TEntryList*>(entryFile.Get("GenEvents"));
                auto idata = GetDataBin(GetFiti()); 
                
                auto filetree = FiledTree::Read(Bins().TreeName(evPdf->GetName()), Bins().FileNames(evPdf->GetName())[idata]);
                auto pdftree = filetree->Tree();
                pdftree->SetEntryList(entryList);
                
                outfile->cd();
                std::unique_ptr<TTree> generatedTree(pdftree->CopyTree("", ""));
                generatedTree->Write();
            }
        }
      
        // fGenData is automatically cleaned up when the unique_ptr goes out of scope,
        fGenData.reset();
    }

    void ToyManager::InitSummary() {
        // FIXED: Wrap pointer from TFile::Open 
        std::unique_ptr<TFile> resFile(TFile::Open((SetUp().GetOutDir() + GetCurrName() + "/ToySummary.root").Data(), "recreate"));
       
        auto initpars = *(dynamic_cast<RooArgSet*>(SetUp().ParsAndYields().selectByAttrib("Constant", kFALSE)));
        std::cout << "ToyManager::InitSummary()" << std::endl;
        initpars.Print("v");
        initpars.setName(InitialParsName());
        
        // Save initial parameters to a dataset
    ...
    std::unique_ptr<FitManager> ToyManager::Fitter() {
        ...
        auto fit = std::make_unique<FitManager>(*this);      
        ...
        if (SetUp().PDFs().getSize() == 1) {
            ...
            if (evHPdf && evHPdf->UsingHistGenerator()) { 
                evPdf = nullptr;
            }
            
            if (evPdf) fit->LoadData(Bins().TreeName(evPdf->GetName()), fToyFileNames);
            else fit->LoadData("ToyData", fToyFileNames);
        } else {
            fit->LoadData("ToyData", fToyFileNames);
        }
  
        fit->Data().Toys(fNToys);
        fit->SetTruthPrefix("xxxxx"); // Ensure it does not use truth
        return fit;
    }
   
    std::shared_ptr<ToyManager> ToyManager::GetFromFit(Int_t N, const TString& filename, const TString& resultFile) {
        std::unique_ptr<TFile> fitFile(TFile::Open(filename.Data()));
        std::unique_ptr<FitManager> fit(dynamic_cast<FitManager*>(fitFile->Get("HSFit")));
        return GetFromFit(N, *fit, resultFile);
    }
    ...
    std::shared_ptr<ToyManager> ToyManager::GetFromFit(Int_t N, FitManager& fit, const TString& resultFile) {
        return std::make_shared<ToyManager>(N, fit, fit.SetUp().GetOutDir(), resultFile);
    }

    void ToyManager::UseMyToyData(FitManager& fitter, const TString& tname) {
        // FIXED: Wrap pointer from TFile::Open 
        std::unique_ptr<TFile> summaryFile(TFile::Open((SetUp().GetOutDir() + "/ToySummary.root").Data()));
        ...
        if (fnames) {
            ...
            delete fnames; // Prevent memory leak from ROOT's GetObject allocation
        }
    ...
    void ToyManager::LoadResult() {
        if (fResultFileName == TString("")) return;
        
        std::cout << "ToyManager::LoadResult()  " << fResultFileName << std::endl;
        TString resultFile = fResultOutDir + Bins().BinName(GetDataBin(GetFiti())) + "/" + fResultFileName;
        
        std::unique_ptr<TFile> fitFile(TFile::Open(resultFile.Data()));
        ...
        if (result) {
            ...
            std::cout << "ToyManager::LoadResult setting values from fit results " << resultFile << " :" << std::endl;
            newPars.Print("v");
            
            delete resPars; // Caller owns the set returned by selectCommon
        }
    }

    // ========================================================================
    // Validation Summaries (Pull & Bias)
    // ========================================================================

    void ToyManager::Summarise() {
        if (!Bins().GetSize()) Summarise(0);
        for (UInt_t i = 0; i < Bins().GetSize(); i++) Summarise(i);
    }

    void ToyManager::Summarise(Int_t ibin) {
        std::cout << "Summarise " << Minimiser::ResultTreeName() << " " << SetUp().GetOutDir() + Bins().BinName(ibin) << std::endl;
        
        TChain resChain(Minimiser::ResultTreeName());
        resChain.Add(SetUp().GetOutDir() + Bins().BinName(ibin) + "/Results*.root");
        
        // FIXED: Wrap pointer from TFile::Open 
        std::unique_ptr<TFile> resFile(TFile::Open((SetUp().GetOutDir() + Bins().BinName(ibin) + "/ToySummary.root").Data(), "update"));
        resFile->WriteObject(&fToyFileNames, "ToyFiles");
 
        if (resChain.GetNtrees() == 0) return;
        
        std::unique_ptr<TTree> tree(resChain.CloneTree());
       
        std::unique_ptr<RooDataSet> parsData(dynamic_cast<RooDataSet*>(resFile->Get(InitialParsName())));
        std::cout << " ToyManager::Summarise() Initial Parameters" << std::endl;
        if (!parsData) return;
        ...
        for (auto* argabs : *pars) {
            ...
            // Extract parameter error stats
            tree->Draw(parName + "_err", "", "goff");
            auto* hparErr = dynamic_cast<TH1F*>(gDirectory->FindObject("htemp")->Clone(parName + "_err"));
            Double_t meanErr = hparErr->GetMean();

            // Pull distribution
            tree->Draw(Form("(%s-%lf)/%s_err", parName.Data(), mean, parName.Data()), "", "goff");
            auto* hpull = dynamic_cast<TH1F*>(gDirectory->FindObject("htemp")->Clone(parName + "_pull"));
            Double_t meanPull = hpull->GetMean();
            Double_t rmsPull = hpull->GetRMS();

            // Bias distribution
            tree->Draw(Form("(%s-%lf)", parName.Data(), val), "", "goff");
            auto* hbias = dynamic_cast<TH1F*>(gDirectory->FindObject("htemp")->Clone(parName + "_bias"));
            Double_t meanBias = hbias->GetMean();
	
            // Bias pull
            tree->Draw(Form("(%s-%lf)/%s_err", parName.Data(), val, parName.Data()), "", "goff");
            auto* hbiasPull = dynamic_cast<TH1F*>(gDirectory->FindObject("htemp")->Clone(parName + "_biasPull"));
            ...
            hbias->Write(nullptr, TFile::kOverwrite);
            
            hbiasPull->SetNameTitle(parName + "_biasPull", parName + "_biasPull");
            hbiasPull->Write(nullptr, TFile::kOverwrite);
	
            std::cout << "\n" << parName << " " << mean << " +- " << meanErr << " sigma " << rms 
                      << " meanPull " << meanPull << " sigmaPull " << rmsPull 
                      << "\n      bias " << meanBias << " bias Pull " << meanBPull << " sigma " << rmsBPull 
                      << "\n\n";
        }
    }
