  ...
  namespace FIT{
	...
	{//Loop over parameters twice to draw corner plot
	 //fix the first parameter 
      ...
      ///////////////////////////////////////////////
      // Start new canvas, without drawing hist range
      ///////////////////////////////////////////////

      auto canvas2 = new TCanvas(canName, canName);
      
      fCanvases->Add(canvas2);
      canvas2->Divide(Npars, Npars);
      ...
      tree->UseCurrentStyle();
      Int_t counter2=0;//Counter for new line on canvas      
	 ...
	 //fix the first parameter 
	  ...
	  for (RooAbsArg* ipar2 : pars)
	    {//second loop 
	      ...
	      if(ipar==ipar2)
		{
		  auto can = canvas2->cd(Npars*counter2+int_counter); 
		  can->SetBorderSize(0);
		  can->SetTopMargin(0.0);
		  can->SetBottomMargin(0.1);
		  can->SetLeftMargin(0.19);
